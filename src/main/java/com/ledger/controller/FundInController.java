...
public class FundInController {
    ...
    @PostMapping
	public ResponseEntity<BaseResponse> fundIn(@RequestBody List<FundInCommand> commands) {
		
		try {
			accountAggregate.handleFundInCommand(commands);
			...
			return new ResponseEntity<>(new BaseResponse("Error when fund in"), HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
}
